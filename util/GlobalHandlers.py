...
def command_wrapper(perm_lvl = 2, log = True, allowed_on_server = True, allowed_in_dm = False):
    def func_receiver(func):
        def func_wrapper(*args, **kwargs):
            ...
            if not allowed:
                log_to_bot_log(plugin.bot,
                               f":warning: {event.msg.author} (``{event.msg.author.id}``) tried to use a command they do not have permission to use: ``{sanitize.S(event.msg.content, escape_codeblocks=True)} ``")
                ...
                if (event.guild is not None and allowed_on_server) or (event.guild is None and allowed_in_dm):
                    try:
                        ...
                        if log:
                            log_to_bot_log(plugin.bot,
                                           f":wrench: {event.msg.author} executed a command: {sanitize.S(event.msg.content, escape_codeblocks=True)}")

...
from config_mock import AnnounceBotConfig, TestPluginConfig
...
class MentorPlugin(Plugin):
    ...
    def get_avail_mentors(self):
       return [u.user.id for u in self.bot.client.state.guilds.get(487611837664198679).members.values() if MENTOR_ID in u.roles and u.user.presence.status != "OFFLINE"]
    ...
    def concat_name_discrim(self, name, discrim):
        return "{}#{}".format(name, discrim)
    ...
    def on_help_command(self, event, content):
        if event.channel.is_dm:
            ...
            if mentors_available:
                self.ping_mentor(mentors_available[random.randint(0, len(mentors_available) - 1)], self.concat_name_discrim(event.msg.author.username, event.msg.author.discriminator), content)
            else:
